...
static const char cmd_get_screen[5] = {0x51, 0x02, 0x04, 0x1f, 0xfc};

static const char cmd_get_version[5] = {0x51, 0x02, 0x04, 0x1f, 0xf8};

static const char cmd_get_id[5] = {0x51, 0x02, 0x08, 0x1f, 0xf0};

static const u8 cmd_draw_part_init[12] = {0x00, 0x2c, 0x00, 0x00, 0x00, 0x00,
                                           0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

static const u8 cmd_quit_sleep[6] = {0x00, 0x29, 0x00, 0x00, 0x00, 0x00};

static const u8 cmd_set_brightness_init[8] = {0x00, 0x51, 0x02, 0x00,
                                                0x00, 0x00, 0x00, 0x00};
...
static int mpro_data_alloc(struct mpro_device *mpro) {
  u64 sz = (u64)mpro->height * mpro->width * MPRO_BPP / 8 + mpro->margin;
  size_t block_size;

  if (sz > SIZE_MAX)
    return -EOVERFLOW;

  block_size = (size_t)sz;
...
static int mpro_send_command(struct mpro_device *mpro, const void *cmd,
                             unsigned int len) {
  ...
  return usb_control_msg(udev, usb_sndctrlpipe(udev, 0), 0xb0, 0x40, 0, 0,
                         mpro->cmd, len, MPRO_MAX_DELAY);
}

static int mpro_set_backlight(struct mpro_device *mpro, int v) {
  u8 cmd[sizeof(cmd_set_brightness_init)];
  int ret;

  memcpy(cmd, cmd_set_brightness_init, sizeof(cmd));
  cmd[6] = v & 0xff;
  ret = mpro_send_command(mpro, cmd, sizeof(cmd));
  return ret < 0 ? ret : 0;
}
...
static int mpro_update_frame(struct mpro_device *mpro, const u8 *draw_cmd,
                             unsigned int len) {
  struct usb_device *udev = mpro_to_usb_device(mpro);
  int ret;

  ret = mpro_send_command(mpro, draw_cmd, sizeof(cmd_draw_part_init));
  if (ret < 0)
    return ret;

  return usb_bulk_msg(udev, usb_sndbulkpipe(udev, 0x02), mpro->draw_buf, len,
                      NULL, MPRO_MAX_DELAY);
}
...
static int mpro_buf_copy(void *dst, struct iosys_map *src_map,
  ...
  drm_fb_xrgb8888_to_rgb565(&dst_map, NULL, src_map, fb, clip, fmtcnv_state,
                            false);
...
static void mpro_fb_mark_dirty(struct iosys_map *src,
                               struct drm_framebuffer *fb,
                               struct drm_rect *rect,
                               struct drm_format_conv_state *fmtcnv_stat) {
  struct mpro_device *mpro = to_mpro(fb->dev);
  u8 draw_cmd[sizeof(cmd_draw_part_init)];
  ...
  draw_cmd[2] = (u8)(len >> 0);
  draw_cmd[3] = (u8)(len >> 8);
  draw_cmd[4] = (u8)(len >> 16);

  ret = mpro_update_frame(mpro, draw_cmd, len);
err_msg:
  if (ret)
 ...
 */
static const struct edid mpro_edid_template = {
    ...
    .detailed_timings =
        {{
         ...
         },
         {
             ...
             .data.other_data.data.range.formula.cvt = {0xa0, 0x20, 0x20, 0x20,
                                                        0x20, 0x20, 0x20},
         },
         {
             .pixel_clock = 0,
             .data.other_data.type = 0xfc, /* Model string */
             .data.other_data.data.str.str = {'M', 'P', 'R', 'O', '\n', ' ',
                                              ' ', ' ', ' ', ' ', ' ', ' ',
                                              ' '},
         },
         {
...
static int mpro_conn_get_modes(struct drm_connector *connector) {
  ...
  drm_connector_update_edid_property(connector, &mpro->edid);
  return drm_add_edid_modes(connector, &mpro->edid);
}
...
static int mpro_conn_late_register(struct drm_connector *connector) {
  ...
  bl = backlight_device_register("mpro_backlight", connector->kdev, mpro,
                                 &mpro_bl_ops, NULL);
  if (IS_ERR(bl)) {
...
static int mpro_conn_init(struct mpro_device *mpro) {
  ...
  return drm_connector_init(&mpro->dev, &mpro->conn, &mpro_conn_funcs,
...
static const uint64_t mpro_pipe_modifiers[] = {DRM_FORMAT_MOD_LINEAR,
...
static int mpro_get_screen(struct mpro_device *mpro) {
  ...
  ret = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0xb6, 0xc0, 0, 0, cmd,
                        1, MPRO_MAX_DELAY);
  if (ret < 1)
    goto err;

  ret = usb_control_msg(udev, usb_rcvctrlpipe(udev, 0), 0xb7, 0xc0, 0, 0, cmd,
                        5, MPRO_MAX_DELAY);
  if (ret < 5)
    goto err;
...
static void mpro_edid_setup(struct mpro_device *mpro) {
  ...
  e->detailed_timings[0].data.pixel_data.hactive_lo = width % 256;
  e->detailed_timings[0].data.pixel_data.hactive_hblank_hi &= 0x0f;
  e->detailed_timings[0].data.pixel_data.hactive_hblank_hi |=
      ((u8)(width / 256) << 4);

  e->detailed_timings[0].data.pixel_data.vactive_lo = height % 256;
  e->detailed_timings[0].data.pixel_data.vactive_vblank_hi &= 0x0f;
  e->detailed_timings[0].data.pixel_data.vactive_vblank_hi |=
      ((u8)(height / 256) << 4);

  e->detailed_timings[0].data.pixel_data.width_mm_lo = width_mm % 256;
  e->detailed_timings[0].data.pixel_data.height_mm_lo = height_mm % 256;
  e->detailed_timings[0].data.pixel_data.width_height_mm_hi =
      ((u8)(width_mm / 256) << 4) | ((u8)(height_mm / 256) & 0xf);

  memcpy(&e->detailed_timings[2].data.other_data.data.str.str, mpro->model,
         strlen(mpro->model));

  snprintf(buf, sizeof(buf), "%02X%02X%02X%02X\n", mpro->id[4], mpro->id[5],
           mpro->id[6], mpro->id[7]);

  memcpy(&e->detailed_timings[3].data.other_data.data.str.str, buf,
         strlen(buf));

  e->checksum = mpro_edid_block_checksum((u8 *)e);
}
...
static int mpro_touch_init(struct usb_interface *interface,
  ...
  mpro->trans_buf =
      usb_alloc_coherent(udev, MPRO_INPUT_TRS_SIZE, GFP_KERNEL, &mpro->dma);
  if (!mpro->trans_buf) {
    ret = -ENOMEM;
    goto error_free_urb;
  }

  usb_fill_int_urb(mpro->irq, udev, usb_rcvintpipe(udev, 1), mpro->trans_buf,
                   MPRO_INPUT_TRS_SIZE, mpro_touch_irq, mpro, 0);
  mpro->irq->dev = udev;
...
static int mpro_usb_probe(struct usb_interface *interface,
  ...
  ret = drm_simple_display_pipe_init(
      &mpro->dev, &mpro->pipe, &mpro_pipe_funcs, mpro_pipe_formats,
      ARRAY_SIZE(mpro_pipe_formats), mpro_pipe_modifiers, &mpro->conn);
  if (ret)
    goto err_put_device;
